...
class Injector extends EventEmitter {
    constructor() {
		...
		this.container = {};
        this.plan = {};
        this.on("replace", (previous, current) => {
            previous.unmount();
        });
	};
	...
	inject(name, service, options = {}) {
		...
		if (name in this.container) {
			...
			if (previous.instance && item.instance && previous.instance == item.instance) return item; // same instance nothing to replace
			if (previous.location && item.location && previous.location == item.location) return item; // same file nothing to replace
            if (/false/i.test(options.replace)) return item; 
            this.container[name] = item;
            this.emit("replace", (previous, item));
		}
