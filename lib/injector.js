...
const { getConstructorArguments } = require("./core/js-parser");
const { camelCaseToDash } = require("./core/string");
...
class Injector {
	...
	inject(name, service, options = {}) {
		...
		if (name in this.container) {
            ...
            if (this.loaded && previous.mounted) throw new Error(`Failed to replace the service ${name} beacause it has been already mounted.`);
