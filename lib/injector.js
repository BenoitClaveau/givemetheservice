...
class Injector extends EventEmitter {
	...
	inject(name, location, options = {}) {
		...
		if (name in this.container) {
			...
			if (previous.instance == item.instance) return item; // same instance nothing to replace
			if (previous.location == item.location) return item; // same file nothing to replace
