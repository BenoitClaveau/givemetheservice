...
class Item {
	...
	constructor(injector, name, service, options = {}) {
		...
		if (typeof service == "object") {
			...
			const relative = this.location[0] == ".";
			let object;
			try {
				this.location = relative ? path.resolve(this.injector.root, this.location) : this.location;
		...
		// Load neasted services
		if (this.instance) this.injectNeastedServices(this.instance.constructor.services);
		else if (this.description) this.injectNeastedServices(this.description.services);
	};

	// inject neasted services declared in static *services
	injectNeastedServices(services) {
		if (!services) return;
		for (const item of services()) {
			...
			this.injector.inject(name, location, { instanciate, reload, replace, mount });
		}
	}
